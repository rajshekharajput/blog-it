...
suite('git smoke test', function () {
    ...
    suiteSetup(async function () {
        ...
        if (git.repositories.length === 0) {
            await (0, util_1.eventToPromise)(git.onDidOpenRepository);
        }
    ...
    test('reflects working tree changes', async function () {
        ...
        repository.state.workingTreeChanges.some(r => r.uri.path === appjs.uri.path && r.status === 5);
        ...
        repository.state.workingTreeChanges.some(r => r.uri.path === appjs.uri.path && r.status === 5);
        repository.state.workingTreeChanges.some(r => r.uri.path === newfile.uri.path && r.status === 7);
    });
    ...
    test('stages correctly', async function () {
        ...
        assert.strictEqual(repository.state.workingTreeChanges.length, 1);
        repository.state.workingTreeChanges.some(r => r.uri.path === newfile.path && r.status === 7);
        assert.strictEqual(repository.state.indexChanges.length, 1);
        repository.state.indexChanges.some(r => r.uri.path === appjs.path && r.status === 0);
        await vscode_1.commands.executeCommand('git.unstage', appjs);
        assert.strictEqual(repository.state.workingTreeChanges.length, 2);
        repository.state.workingTreeChanges.some(r => r.uri.path === appjs.path && r.status === 5);
        repository.state.workingTreeChanges.some(r => r.uri.path === newfile.path && r.status === 7);
    });
