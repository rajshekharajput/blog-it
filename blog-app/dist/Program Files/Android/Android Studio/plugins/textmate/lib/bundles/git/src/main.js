...
async function createModel(context, outputChannel, telemetryReporter, disposables) {
    ...
    const info = await (0, git_1.findGit)(pathHint, path => outputChannel.appendLine(localize('looking', "Looking for git in: {0}", path)));
    ...
    disposables.push(cc, new fileSystemProvider_1.GitFileSystemProvider(model), new decorationProvider_1.GitDecorations(model), new protocolHandler_1.GitProtocolHandler(), new timelineProvider_1.GitTimelineProvider(model, cc));
